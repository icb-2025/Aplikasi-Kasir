...
import type { Pesanan, BarangDibeli } from ".";
...
const PesananTable: React.FC<PesananTableProps> = ({ data, onUpdateStatus, loading = false }) => {
  ...
  const [selectedBarangList, setSelectedBarangList] = useState<BarangDibeli[]>([]);
  ...
  const openDetailModal = (barangList: BarangDibeli[]) => {
    setSelectedBarangList(barangList);
    setShowDetailModal(true);
  };

  const closeDetailModal = () => {
    setShowDetailModal(false);
    setSelectedBarangList([]);
  };
                        ...
                        <div className="space-y-1">
                          <div className="flex justify-between items-center">
                            <span className="text-sm font-medium text-gray-600">
                              {item.barang_dibeli.length} jenis barang
                            </span>
                            <button
                              onClick={() => openDetailModal(item.barang_dibeli)}
                          ...
                          <div className="text-xs text-gray-500">
                            Total: {item.barang_dibeli.reduce((sum, barang) => sum + barang.jumlah, 0)} item
                          </div>
                        </div>
                      ) : (
                        "-"
